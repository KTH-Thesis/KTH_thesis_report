...
Assuming that
\begin{itemize}
  \item all agents $i \in \mathcal{V}$ have access to their own state and
    input vectors
  \item all agents $in \in \mathcal{V}$ have a (upper-bounded) sensing range
   $d_i$ such that
  ...
  \item All obstacles $\ell \in \mathcal{L}$ are situated
    ...
    is larger than the diameter of one agent
\end{itemize}
...
$\vect{z}_{i,des},\underline{d}_{ij,a}, d_i,underline{d}_{i\ell,o}$ and
sets $\mathcal{N}_i$.
\end{problem}
