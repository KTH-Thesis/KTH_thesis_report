...
time instant $\tau$. The form of the constraint regime is necessary to be
such, as each agent is constrained not by constant values, but by
...
Figure \eqref{fig:constraint_regime_horizon} depicts the designed constraint
regime.\\[2.5ex]

\begin{figure}[ht!]
  \centering
  \input{figures/constraint_regime_horizon.tex}
  \caption{The constraint regime for two agents, $i,j$. Fully outlined circles
    denote measured configurations, while partly outlined circles denote
    predicted configurations. During the solution to the individual optimization
    problems, the predicted configuration of each agent at each
    timestep is constrained by the predicted configuration of the other agent
    at the same timestep (hence the identical colours at each discrete timestep).}
...
The terminal set $\Omega_i \subseteq \mathcal{E}_{i,s}$ is an admissible
...
\begin{align}
  \Omega_i = \{\vect{e}_i \in \mathcal{E}_{i,s} : V_i(\vect{e}_i) \leq \varepsilon_{\Omega_i} \}
\end{align}
...
\begin{assumption}
  ...
  \textit{continuous solution} for any $\vect{e}_i(0) \in \mathcal{E}_{i,s}$ and
...
\eqref{problem:opt_without_disturbances}})
  ...
  \begin{enumerate}
    ...
    \item $\overline{\vect{e}}_i\big(\tau;\ \vect{u}_i(\cdot), \vect{e}_i(t_k)\big) \in \mathcal{E}_{i,s},\ \forall \tau \in [t_k, t_k + T_p]$
