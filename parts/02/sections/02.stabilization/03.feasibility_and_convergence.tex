...
\begin{theorem}
  ...
  \begin{enumerate}
    \item the terminal region $\mathcal{E}_{i,f} \subseteq \mathcal{E}_i$ is
      closed with $0 \in \mathcal{E}_{i,f}$
    ...
    \item there exists an admissible control input
      ...
      $\vect{e}_i \in \mathcal{E}_{i,f}$ and $\tau \in [0,h]$:
...
crucially: $\overline{\vect{e}}_i\big(t_k + T_p;\ \vect{u}_i^{\star}(\cdot), \vect{e}_i(t_k)\big) \in \mathcal{E}_{i,f}$.
...
and the input $\vect{u}_{i,f}(\cdot)$:
...
\end{align}
which, due to the fact that $\tau < t$, is equivalent to
\begin{align}
...
term $\int\limits_{0}^{t} \|\vect{e}_{0,i}(s)\|^2 ds$ is bounded as well.
\begin{bw_box}
  \begin{lemma} (\textit{A modification of Barbalat's lemma}\cite{Fontes2007})

    Let $f$ be a continuous, positive-definite function, and $x$ be an
    absolutely continuous function in $\mathbb{R}$. If the following hold:
  \begin{itemize}
    \item $\|x(\cdot)\| < \infty, \|\dot{x}(\cdot)\| < \infty$
    \item $\lim\limits_{t \to \infty} \int\limits_0^t f\big(x(s)\big) < \infty$
  \end{itemize}
  then $\lim\limits_{t \to \infty} \|x(t)\| = 0$
  \label{lemma:barbalat}
  \end{lemma}
\end{bw_box}
Lemma \eqref{lemma:barbalat} assures us that under these conditions for the
error and its dynamics, the error
$$\lim\limits_{t \to \infty}\|\vect{e}_{0,i}(t)\| \to 0$$
which means that
