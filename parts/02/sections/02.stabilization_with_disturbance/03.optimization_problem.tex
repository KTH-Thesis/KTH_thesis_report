...
under some considerations, see \cite{Fontes2007} for more.}, force the states
...
Let us define for convenience $\vect{\zeta}_i \in \mathbb{R}^9 \times \mathbb{T}^3$:
...
\end{align}
which means that $\vect{\zeta}_i(s) \in \mathcal{B}_{s-t_k}$. If
$\vect{\zeta}_i(s)$ resides in $\mathcal{B}_{s-t_k}$, then
$-\vect{\zeta}_i(s)$ also resides in $\mathcal{B}_{s-t_k}$.
...
positively invariant set for system \eqref{eq:position_based_error_model_2}
such that
\begin{align}
  \mathcal{E}_{i,f} = \{\vect{e}_i \in \mathcal{E}_i : V_i(\vect{e}_i) \leq \varepsilon_0 \}
\end{align}
