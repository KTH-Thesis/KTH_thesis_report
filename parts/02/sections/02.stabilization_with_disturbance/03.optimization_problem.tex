...
\begin{align}
  \text{Find }& \\
    ...
    J_i \big(\vect{e}_i(t_k), \overline{\vect{u}}_i (\cdot) \big) \label{position_based_cost_2} \\
    \text{where}& \\
...
$V_i: \Phi_i \to \mathbb{R}_{\geq 0}$ and matrices
$\mat{R}_i \in \mathbb{R}^{6 \times 6}$,
$\mat{Q}_i, \mat{P}_i \in \mathbb{R}^{12 \times 12}$, the same things hold
as in the disturbance-free case. With regard to the terminal penalty function $V_i$, we assume that it is
Lipschitz continuous in $\Psi_i$ with Lipschitz constant
$L_{V_i} = 2 \varepsilon_{\Psi_i} \lambda_{max}(P_i)$.
