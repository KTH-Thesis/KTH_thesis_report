...
\begin{align}
  ...
  &\leq h \overline{\delta}_i
     + L_{g_i} \int_{t_k}^{t_{k+1}} \bigg\| \vect{e}_i\big(s;\ \overline{\vect{u}}_i^{\star}\big(\cdot;\ \vect{e}_i(t_k)\big), \vect{e}_i(t_k)\big) -
...
\begin{bw_box}
  \begin{lemma}
    Suppose that the real system and the model are both at time $t_k$ at state
    $\vect{e}_i(t_k)$. Applying at time $t_k$ a control law $\vect{u}(\cdot)$
    to the real system and the model will cause at time $t_k + \tau$,
    $\tau \geq 0$ a divergence between the states of the real system and the
    model. The norm of the difference between the state of the real system
    and the state of the model is bounded by
    \begin{align}
