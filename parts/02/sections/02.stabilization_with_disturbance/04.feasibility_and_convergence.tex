...
Finally, $\overline{\vect{e}}_i\big(t_{k+1} + s;\ \widetilde{\vect{u}}_i(\cdot), \vect{e}_i(t_{k+1})\big) \in \mathcal{E}_i \ominus \mathcal{B}_{s}$,
for all $s \in [0,  T_p]$
...
\end{align}
But\footnote{PROOF} $(A \ominus B) \oplus (B \ominus C) = (A \oplus B) \ominus (B \oplus C)$
