...
Piecing the two last statements together, we conclude that the application of
the control input $\widetilde{u}_i(\cdot)$ at time $t_{k+1}$ results in
the states of the real system abiding by their intended state constraints
across the entire horizon $t_{k+1}, t_{k+1} + T_p$.

Therefore, overall, the (sub-optimal) control input $\widetilde{u}_i(\cdot)$
is admissible at time $t_{k+1}$, which means that feasibility of a solution to
the optimization problem at time $t_k$ implies feasibility at time
