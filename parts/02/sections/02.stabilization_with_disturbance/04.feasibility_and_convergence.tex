...
In other words
\begin{align}
\bigg\| &\big(\overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big) - \vect{e}_i(t_{k+1}+s)\big)
  -\big(\overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_k)\big) - \vect{e}_i(t_{k+1}+s)\big)\bigg\| \\
  &\leq \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big)
\end{align}
Using the inverse triangular inequality\footnote{$\forall x,y \in \mathbb{R}: \bigg| \|x\| - \|y\| \bigg| \leq \| x-y\|$} we get
\begin{align}
  \Bigg| \bigg\| & \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  -\bigg\| \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_k)\big) \bigg\| \Bigg| \\
  &\leq \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big)
\end{align}
By utilizing lemma \eqref{lemma:diff_state_from_same_conditions} once more
with regard to the second term in the left-hand side we get:
\begin{align}
  \Bigg| \bigg\| & \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  -\dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(t_{k+1} + s -t_k)}\big) \Bigg|\\
  &\leq \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big)
\end{align}
but since $t_{k+1}+s - t_k = t_k + h + s - t_k = h+s$
\begin{align}
  \Bigg| \bigg\| & \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  -\dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(s+h)}\big) \Bigg|\\
  &\leq \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big) \\
  &\leq \bigg\| \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big) \bigg\|
\end{align}
since $x \leq \|x\|, \forall x \in \mathbb{R}$. Expanding we get
\begin{align}
  &-\bigg\| \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big) \bigg\| \\
  &\leq \bigg\| \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  -\dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(s+h)}\big) \\
  &\leq \bigg\| \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big) \bigg\|
\end{align}
Expanding once more for the right-hand side inequality, we get
\begin{align}
  & \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big) \\
  &\geq \bigg\| \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  -\dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(s+h)}\big) \\
  &\text{or} \\
  & \dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(h+s)} -1\big) - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big) \\
  &\leq -\bigg\| \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  +\dfrac{\overline{\delta}_i}{L_{g_i}} \big(e^{L_{g_i}(s+h)}\big)
\end{align}
Removing the equal terms, the last inequality yields
\begin{align}
  - \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big)
    \leq -\bigg\| \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|\\
    \bigg\| \vect{e}_i(t_{k+1}+s) - \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big)\bigg\|
  \leq \dfrac{\overline{\delta}_i}{L_{g_i}}\big(e^{L_{g_i} s}-1\big)
\end{align}
Notice that we could have arrived at the same result by direct application of
lemma \eqref{lemma:diff_state_from_same_conditions} for $t=t_{k+1}$ and
$\tau = s$.
...
where
\begin{align}
  \vect{a}_i &= \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_k)\big) - \vect{e}_i(t_{k+1}+s) \\
  \vect{b}_i &= \overline{\vect{e}}_i\big(t_{k+1} + s;\ \overline{\vect{u}}_i^{\star}(\cdot), \vect{e}_i(t_{k+1})\big) - \vect{e}_i(t_{k+1}+s)
\end{align}
