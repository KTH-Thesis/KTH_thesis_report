...
\begin{theorem}
  ...
  \begin{enumerate}
    ...
    \item there exists an admissible control input
      $\vect{h}_i(\vect{u}_i) : [t_k + T_p, t_{k+1} + T_p] \to \mathcal{U}_i$
      ...
      \begin{enumerate}
        ...
        \vect{h}_i\big(\vect{u}_i(\tau)\big)
          + F_i\big(\vect{e}_i(\tau), \vect{h}_i\big(\vect{u}_i(\tau)\big)\big) \leq 0$
      \end{enumerate}
...
admissible (and certainly not guaranteed optimal) input $\vect{h}_i(\vect{u}_i)$ that
...
\begin{equation}
  \widetilde{\vect{u}}_i(\tau) = \left\{
      \begin{array}{ll}
        ...
        \vect{h}_i\big(\vect{u}_i(\tau)\big), & \tau \in (t_k + T_p, t_{k+1} + T_p]
      \end{array}
      \right.
...
to the terminal set $\Omega_i$ as $t \to \infty$.\\ \qedsymbol
