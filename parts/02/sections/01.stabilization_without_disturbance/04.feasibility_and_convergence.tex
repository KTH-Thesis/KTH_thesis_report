...
Applied at time $t_{k+1}$, $\widetilde{\vect{u}}_i\big(\cdot;\ \vect{e}_i(t_{k+1})\big)$
...
\begin{itemize}
  ...
  \item $\vect{e}_{0,i}(\tau) \triangleq \overline{\vect{e}}_i\big(\tau;\ \vect{u}_{0,i}(\tau), \vect{e}_i(t_k)\big)$
    ...
    $\overline{\vect{u}}_i^{\star}\big(\tau;\ \vect{e}_i(t_k)\big)$ at time $\tau$
  \item $\vect{u}_{1,i}(\tau) \triangleq \widetilde{\vect{u}}_i\big(\tau;\ \vect{e}_i(t_{k+1})\big)$
  ...
  \item $\vect{e}_{1,i}(\tau) \triangleq \overline{\vect{e}}_i\big(\tau;\ \vect{u}_{1,i}(\tau), \vect{e}_i(t_{k+1})\big)$
    ...
    the state that results from the application of the above input
    $\widetilde{\vect{u}}_i\big(\tau;\ \vect{e}_i(t_{k+1})\big)$ at time $\tau$
\end{itemize}
\end{gg_box}
...
$\widetilde{\vect{u}}_i\big(\cdot;\ \vect{e}_i(t_{k+1})\big)$ is equal to the optimal
