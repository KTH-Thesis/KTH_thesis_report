...
\label{lemma:V_Lipschitz_e_0}
  ...
  with Lipschitz constant $L_{V_i} = 2 \varepsilon_{\Omega_i} \lambda_{max}(\mat{P}_i)$\\
...
\begin{align}
  |V(\vect{x}) - V(\vect{y})| &= |\vect{x}^{\top} \mat{P}_i \vect{x} - \vect{y}^{\top} \mat{P}_i \vect{y}| \\
    &= |\vect{x}^{\top} \mat{P}_i \vect{x} - \vect{y}^{\top} \mat{P}_i \vect{y} \pm \vect{x}^{\top} \mat{P}_i \vect{y}| \\
    &= |\vect{x}^{\top} \mat{P}_i (\vect{x} -\vect{y}) - \vect{y}^{\top} \mat{P}_i (\vect{x}-\vect{y})| \\
...
\begin{align}
  ...
  \sigma_{max}(\mat{P}_i) \|\vect{y}\| \|\vect{x} - \vect{y}\| \\
  &= \sigma_{max}(\mat{P}_i) (\|\vect{x}\| + \|\vect{y}\|)\|\vect{x} - \vect{y}\| \\
  & \leq \sigma_{max}(\mat{P}_i) (\varepsilon_{\Omega_i} + \varepsilon_{\Omega_i})\|\vect{x} - \vect{y}\| \\
...
\end{align}
At time $t_{k+1}$ a new optimal control problem is solved in the same manner,
leading to a receding horizon approach.
...
\begin{align}
  \vect{e}_i(\tau) = g_i\big(\vect{e}_i(\tau), \overline{\vect{u}}_i^{\star}(\tau)\big),\ \tau \in [t_k, t_k + h)
