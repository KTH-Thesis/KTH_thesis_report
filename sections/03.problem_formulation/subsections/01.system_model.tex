...
\begin{figure}[ht!]
    ...
    \caption{Illustration of two agents $i, j \in \mathcal{V}$ and an static
      obstacle $k \in K$ in the workspace; $\{\mathcal{O}\}$ is the inertial
...
In the workspace there are $|K|$ \textit{static obstacles}, modeled as
spheres with centers at positions $\vect{p}_{k}$ with radii
$r_{k}\in \mathbb{R}^3, k \in K = \{1,\dots,|K| \}$.
Thus, the obstacles are modeled by the spheres
$\mathcal{B}(\vect{p}_{k}, r_{k}), k \in \{1,\dots,|K|\}$. The geometry in
...
\begin{align*}
  d_o &\triangleq \min\{\| \vect{p}_{k} - \vect{p}_{k'}\| : k,k' \in K, k \neq k' \},
\end{align*}
as the distance between the two least distant obstacles in the workspace,
$d_{o,W}$
\begin{align*}
  d_{o,W} & \triangleq \min\{r_W - \left( r_{k} + \| \vect{p}_{k} \| \right) : k \in K\},
\end{align*}
