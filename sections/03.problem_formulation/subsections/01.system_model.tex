...
\begin{subequations}
    ...
    d_{iz,o}(t) &\triangleq \| \vect{p}_i(t) - \vect{p}_z(t) \|
	\end{align}
\end{subequations}

$\forall i, j \in \mathcal{V}, i \neq j, z \in Z$, as well as
constants $\underline{d}_{ij, a} \triangleq r_{i} + r_{j},
\underline{d}_{iz, o} \triangleq r_{i} + r_{o_z}$. The latter stand for
...
\begin{align}
\mathcal{E}=\big\{\{i,j\}\in\mathcal{V}\times\mathcal{V} :
  i \neq j,
...
Equation \ref{eq:edges_set} implies that the graph is undirected, i.e.,
\begin{align}
...
\begin{align*}
  d_o &\triangleq \min\{\| \vect{p}_{o_z} - \vect{p}_{o_{z'}}\| : z,z' \in Z, z \neq z' \},
\end{align*}
as the distance between the two least distant obstacles in the workspace,
$d_{o,W}$
\begin{align*}
  d_{o,W} & \triangleq \min\{r_W - \left( r_{o_z} + \| \vect{p}_{o_z} \| \right) : z \in Z\},
\end{align*}
